...
let rec build (rand,depth) = match depth with
  ...
  | _		-> let i = rand(0, 5) in match i with
		...
		| _ -> failwith "this should never happen, is your random number
				generator working properly?"

let rec build2 (rand,depth) = match depth with
  | 0		-> if (rand(0,2) = 0) then buildX() else buildY()
  | _		-> let i = rand(0, 7) in match i with
		...
		| 4 -> buildThresh(build2(rand, depth-1), build2(rand, depth-1),
				   build2(rand, depth-1), build2(rand, depth-1))
		| 5 -> buildMagicCat(build2(rand, depth-1))
		| 6 -> buildMagicDog(build2(rand, depth-1), build2(rand, depth-1),
				     build2(rand, depth-1))
		| _ -> failwith "this should never happen, is your random number
				generator working properly?"
...
let c1 () = (9, 322, 500)
let c2 () = (9, 12000, 12000) 
let c3 () = (9, 99999999, 88888888)
